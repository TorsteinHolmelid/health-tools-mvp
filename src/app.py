...
def create_pdf_bytes_ultimate(report: dict) -> bytes:
    ...
    # Hjelpefunksjonar for tekst (lokale, unngår kollisjon)
    ...
    # Lokale flowables (må definerast inni funksjonen for å ha tilgang til fargekonstantane)
    class VGap(Flowable):
    ...
    class HealthScoreRing(Flowable):
        ...
        def draw(self):
            ...
            for j,(dim,sc) in enumerate(dims):
                ...
                c.setFillColor(MUTED); c.setFont("Helvetica",6); c.drawCentredString(dx,dy,str(dim))
    ...
    class ExpertInsightBox(Flowable):
        def __init__(self, section: str, text: str, width=CONTENT_W):
            super().__init__()
            self.w = width
            self._header = Paragraph(f'<b>🔬 EXPERT INSIGHT — {section.upper()}</b>', S(f"_ei_h_{abs(hash(text))}", size=7.5, lead=11, color=WARN, bold=True))
    ...
    class ActionableMilestoneBox(Flowable):
        def __init__(self, steps: list, width=CONTENT_W):
            ...
            self._header = Paragraph('<b>🎯 ACTIONABLE MILESTONE — YOUR NEXT 4 WEEKS</b>', S(f"_am_h_{abs(hash(bullet_html))}", size=7.5, lead=11, color=ACCENT, bold=True))
    ...
    class CompoundingEffectBox(Flowable):
        def __init__(self, width=CONTENT_W):
            super().__init__()
            self.w = width
            body_html = ("<b>Health is compound interest.</b> A 1% weekly improvement in sleep quality, training load, or nutrition precision compounds to a <b>52% total gain over one year.</b> The habits you establish today are not just today's result — they are the foundation every future week builds upon. Small, consistent actions have disproportionate long-term returns. This is the defining principle of every intervention recommended in this report.")
            self._header = Paragraph('<b>📈  THE COMPOUNDING EFFECT — WHY 1% MATTERS</b>', S("_ce_h", size=7.5, lead=11, color=BLUE, bold=True))
    ...
    info_rows = [[P("AGE",S("il",size=6.5,color=MUTED,align=TA_CENTER)),P("SEX",S("il",size=6.5,color=MUTED,align=TA_CENTER)),P("HEIGHT",S("il",size=6.5,color=MUTED,align=TA_CENTER)),P("WEIGHT",S("il",size=6.5,color=MUTED,align=TA_CENTER)),P("DATE",S("il",size=6.5,color=MUTED,align=TA_CENTER))],
...
def png_to_rl_image(buf: io.BytesIO, width_pts: float, height_pts: float):
    ...
    _has_strength = bool(set(_activities) & _strength_acts)
    _has_cardio = bool(set(_activities) & _cardio_acts)
    _has_sport = bool(set(_activities) & _sport_acts)
    _has_low = bool(set(_activities) & _low_acts)
    ...
    if not _activities:
        _has_strength = True
        _has_cardio = True
        _sel_strength = ["Strength training (weights)"]
        _sel_cardio = ["Running/jogging"]
        _sel_low = ["Walking (casual)"]
        _activities = ["Strength training (weights)", "Running/jogging"]

    # Målspesifikke parametere
    _goal_params = {
        "Lose fat": {"deficit": -400, "protein": "2.2 g/kg", "goal_color": "#EF4444"},
        "Build muscle (bulk)": {"deficit": +350, "protein": "2.0 g/kg", "goal_color": "#22C55E"},
        "Body Recomposition": {"deficit": 0, "protein": "2.4 g/kg", "goal_color": "#0EA5A3"},
    }
    ...
    # ── Premium header med aktivitetsteller ──
    class TrainingHero(Flowable):
        ...
        def draw(self):
            ...
            c.setStrokeColor(ACCENT)
            c.roundRect(16, 12, pill_w, 24, 12, fill=0, stroke=1)
            c.setFillColor(ACCENT)
            ...
            c.setStrokeColor(ACCENT)
            c.roundRect(16+2*(pill_w+8), 12, pill_w, 24, 12, fill=0, stroke=1)
            c.setFillColor(ACCENT)
            c.drawCentredString(16+2*(pill_w+8)+pill_w/2, 20, f"🏃 {self.act_count} activities")

    story.append(TrainingHero(_goal, _goal_col, _weeks, _protein_g, len(_activities), CONTENT_W))
    story.append(VGap(12))

    # ── Aktivitetssammendrag som en liste med ikoner ──
    if _activities:
        story.append(P("Your Selected Activities", S("sh2", size=10, bold=True, color=ACCENT, after=4)))
        act_list = " · ".join(_activities[:5])
...
# ── Ukentlig timeplan med premium styling ──
    ...
    citations = []
    if _has_strength:
        evidence_text += "• Resistance training stimulates muscle protein synthesis (MPS) and increases resting metabolic rate, directly improving body composition and glucose disposal (Schoenfeld, 2010; J Strength Cond Res).<br/>"
        citations.append("Schoenfeld BJ (2010) – The mechanisms of muscle hypertrophy and their application to resistance training.")
    if _has_cardio:
        evidence_text += "• Aerobic exercise enhances VO2max, the single strongest predictor of all-cause mortality. Even modest increases (3–5 ml/kg/min) reduce mortality risk by 10–15% (Mandsager et al., 2018; JAMA).<br/>"
        citations.append("Mandsager et al. (2018) – Association of cardiorespiratory fitness with long-term mortality among adults undergoing exercise treadmill testing.")
    if _has_sport:
        evidence_text += "• Skill-based sports (table tennis, badminton, tennis) improve executive function, hand-eye coordination, and provide high-intensity intervals that boost cardiovascular fitness (Zagatto et al., 2016; J Sports Sci Med).<br/>"
        citations.append("Zagatto AM et al. (2016) – Physiological responses and characteristics of table tennis matches.")
    if _has_low:
        evidence_text += "• Low-impact activities (walking, yoga) reduce systemic inflammation, improve joint mobility, and contribute significantly to daily energy expenditure (NEAT) without taxing recovery (Levine, 2004; Am J Clin Nutr).<br/>"
        citations.append("Levine JA (2004) – Nonexercise activity thermogenesis (NEAT): environment and biology.")
    ...
    if _goal == "Build muscle (bulk)":
        prog_data = [("1–2", "Neural adaptation", "3×12", "60–65%", "Focus on form & mind-muscle connection"),
                     ...
                     ("5–6", "Hypertrophy B", "4×8", "72–77%", "Track RPE (7–8)"),
                     ("7–8", "Strength blend", "5×6", "77–82%", "Introduce heavier compounds"),
                     ("9–10","Volume peak", "4×10–12", "70–75%", "Max volume – prioritise sleep & nutrition"),
                     ("11", "Intensification", "4×6–8", "80–85%", "Attempt personal bests"),
                     ("12", "Deload", "3×8", "55–60%", "Reduce volume 40% – supercompensation")]
    elif _goal == "Lose fat":
        prog_data = [("1–2", "Metabolic reset", "3×12–15", "55–65%", "Short rests 45–60 s"),
                     ("3–4", "Density A", "3×12", "65–70%", "Circuit format"),
                     ("5–6", "Density B", "4×10", "67–72%", "Add HIIT finisher 15 min"),
                     ("7–8", "Strength bias", "4×8", "72–78%", "Heavier work preserves lean mass"),
        ...
        prog_data = [("1–2", "Foundation", "3×10–12", "60–67%", "Movement quality · establish baseline"),
                     ...
                     ("7–8", "Strength+Vol", "4×8", "75–80%", "Combination block"),
                     ("9–10","Volume peak", "4×10–12", "70–75%", "Highest weekly volume – nutrition critical"),
                     ("11", "Intensification", "4×6", "82–87%", "Max strength stimulus"),
                     ("12", "Deload", "3×8", "55–60%", "Consolidation week")]

    prog_header = [P("WEEK", S("ph", size=7.5, bold=True, color=MUTED, align=TA_CENTER)),
                   P("PHASE", S("ph", size=7.5, bold=True, color=MUTED)),
                   P("SETS×REPS", S("ph", size=7.5, bold=True, color=MUTED, align=TA_CENTER)),
                   P("INTENSITY", S("ph", size=7.5, bold=True, color=MUTED, align=TA_CENTER)),
                   P("KEY FOCUS", S("ph", size=7.5, bold=True, color=MUTED))]
    prog_rows = [prog_header]
    for i, (wk, phase, sets, inten, focus) in enumerate(prog_data):
        prog_rows.append([P(wk, S(f"pw{i}", size=8.5, bold=True, align=TA_CENTER, color=ACCENT)),
                          P(phase, S(f"pp{i}", size=8, color=TEXT)),
                          P(sets, S(f"ps{i}", size=8.5, bold=True, align=TA_CENTER, color=BLUE)),
                          P(inten, S(f"pi{i}", size=8, align=TA_CENTER, color=MUTED)),
                          P(focus, S(f"pf{i}", size=8, lead=11, color=MUTED))])
    prog_t = Table(prog_rows, colWidths=[CONTENT_W*0.12, CONTENT_W*0.22, CONTENT_W*0.18, CONTENT_W*0.18, CONTENT_W*0.30])
    ...
    if _goal == "Build muscle (bulk)":
        train_steps = [f"Track bodyweight weekly – target gain 0.25–0.5 kg/week for first {min(_weeks,8)} weeks",
                       ...
                       "Log every session – add 2.5 kg when you complete all reps with RPE ≤8",
                       "Prioritise 8 hours of sleep – anabolic window for muscle repair"]
    elif _goal == "Lose fat":
        train_steps = [f"Weigh daily (post-toilet) – use 7-day rolling average",
                       f"Minimum {_protein_g} g protein to preserve lean mass in deficit",
                       "Step count ≥8,000/day – NEAT contributes 15–30% of TDEE",
                       "Strength before cardio – muscle drives resting metabolic rate"]
    else:
        train_steps = [f"Eat at maintenance ±100 kcal – weigh weekly and adjust if needed",
                       f"Protein target: {_protein_g} g/day – essential for recomposition",
                       "Progressive overload on 2–3 key lifts (squat, press, row)",
                       "Rotate cardio modality every 4 weeks to prevent adaptation"]
    ...
    story.append(P(
        ...
        "Targets are population estimates – individual responses vary. Consult a physician before starting.",
    ...
    # ════════════════════════════════════════════════════════════
    # EXECUTIVE SUMMARY — TABELLØSNING (enkel og robust)
    # ════════════════════════════════════════════════════════════
    
    # Bygg opp lister (samme som før)
    _stop_items  = []
    _start_items = []
    _keep_items  = []
    ...
    # Sørg for at alle lister har minst 3 elementer
    while len(_stop_items) < 3: _stop_items.append("(Ingen spesifikke anbefalinger akkurat nå)")
    while len(_start_items) < 3: _start_items.append("(Ingen spesifikke anbefalinger akkurat nå)")
    while len(_keep_items) < 3: _keep_items.append("(Ingen spesifikke anbefalinger akkurat nå)")

    # Definer en stil for punktene (bruker Paragraph for automatisk linjebryting)
    ...
    # Lag en kolonne som en liste med Paragraph-objekter
    def make_column(title, emoji, color_hex, items):
        # Header
        header = Paragraph(f'<font color="{color_hex}" size="12"><b>{emoji}  {title}</b></font>', 
                           ParagraphStyle("h", parent=_styles["Normal"], alignment=TA_CENTER, spaceAfter=12))
        # Punkter
        bullet_paras = [Paragraph(f"• {escape(str(item))}", bullet_style) for item in items]
        # Kombiner i en liste
        col_content = [header] + bullet_paras
        # Lag en tabell for kolonnen (for å få bakgrunn og ramme)
        col_table = Table([[p] for p in col_content], colWidths=[CONTENT_W/3 - 10])
        col_table.setStyle(TableStyle([
            ("BACKGROUND", (0,0), (-1,-1), HexColor("#150202" if title=="STOP" else "#011008" if title=="START" else "#020A18")),
    ...
    # Lag de tre kolonnene
    col_stop = make_column("STOP", "🛑", "#EF4444", _stop_items[:3])
    col_start = make_column("START", "🚀", "#22C55E", _start_items[:3])
    col_maintain = make_column("MAINTAIN", "✅", "#3B82F6", _keep_items[:3])

    # Sett dem sammen i en rad med tre kolonner
    summary_table = Table([[col_stop, col_start, col_maintain]], colWidths=[CONTENT_W/3, CONTENT_W/3, CONTENT_W/3])
    ...
    # Legg til på en ny side
    story.append(PageBreak())
    story.append(P("EXECUTIVE SUMMARY — YOUR PERSONAL CHEAT SHEET", 
                   S("es_title", size=14, bold=True, color=ACCENT, align=TA_CENTER, after=6)))
    story.append(P("Review quarterly · Share with your physician · Act on the top priority daily",
                   S("es_sub", size=8, color=MUTED, align=TA_CENTER, after=12)))
    story.append(summary_table)
    story.append(Spacer(1, 12))
    
    # Legg til CompoundingEffectBox og disclaimer på samme side eller ny side
    story.append(CompoundingEffectBox())
    ...
    story.append(P(
        ...
        S("_es_disc", size=7.5, lead=11, color=MUTED, italic=True, align=TA_CENTER, after=4)
    ))
